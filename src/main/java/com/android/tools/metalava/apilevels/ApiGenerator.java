...
public class ApiGenerator {
    private static Map<String, Integer> EXT_NAME_TO_CODE = new HashMap<>();
    static {
        EXT_NAME_TO_CODE.put("R", 30);
        EXT_NAME_TO_CODE.put("S", 31);
        EXT_NAME_TO_CODE.put("T", 33);
    }
    ...
    private static void processExtensionSdkApis(@NotNull Api api, @NotNull File sdkJarRoot, @NotNull File filterPath) throws IOException, IllegalArgumentException {
        ...
        for (Map.Entry<String, List<VersionAndPath>> entry : map.entrySet()) {
            ...
            for (ApiClass sdkClass : sdkApi.getClasses()) {
                ...
                Set<String> extensions = filter.getExtensions(clazz);
                String clazzFromAttr = calculateFromAttr(clazz.getSince(), extensions, sdkClass.getSince());
                ...
                while (iter.hasNext()) {
                    ...
                    if (extensions.isEmpty()) {
                        ...
                        if (sdkField != null) {
                            String from = calculateFromAttr(field.getSince(), extensions, sdkField.getSince());
                ...
                while (iter.hasNext()) {
                    ...
                    if (extensions.isEmpty()) {
                        ...
                        if (sdkMethod != null) {
                            String from = calculateFromAttr(method.getSince(), extensions, sdkMethod.getSince());
                            if (!clazzFromAttr.equals(from)) {
     ...
     *
     * @param androidSince the version of the Android platform in which this API was introduced, or 0 if it is not part of the Android platform
     ...
     */
    @VisibleForTesting
    static String calculateFromAttr(int androidSince, Set<String> extensions, int extensionsSince) {
        List<String> versions = new ArrayList<>();
        if (androidSince > 0) {
            versions.add("0:" + androidSince);
        }
        for (String ext: extensions) {
            Integer code = EXT_NAME_TO_CODE.get(ext);
            if (code == null) {
                throw new IllegalArgumentException("unknown extension SDK \"" + ext + "\"");
            }
            versions.add(code + ":" + extensionsSince);
        }
